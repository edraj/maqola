...
import {type ResponseEntry, ResourceType, ContentType} from "@edraj/tsdmart";
...
export default {
  ...
  load: async (resource_type : ResourceType, space_name: string, subpath: string, shortname: string) => {
    const value = await Dmart.retrieve_entry(resource_type, space_name, subpath, shortname, true, true);
